"""
    Step 2 (2/2) of Gox ML task: classification
        runtime: ~ 12 hours (716m30.145s)
"""
...
feature_dtype_dict = {
    ...
    'buyer_24h_btc_sum': float,
    'buyer_7d_btc_sum': float,
    'seller_24h_btc_sum': float,
    'seller_7d_btc_sum': float,
    'buyer_all_btc_sum': float,
    'seller_all_btc_sum': float,
...
market_path = "data/ml_sample/gox_ml_samples.csv"
results_path = "data/results/gox_ml_metrics.csv"
fi_path = "data/results/gox_ml_feature_importance.csv"
...
def optimize_threshold(y_prob, y_true, X_test, num_thresholds=100):
    """
    Finds the optimal decision threshold that minimizes error
    ...
    - X_test: pd.DataFrame, must include "bitcoins" column.
    ...
    bitcoin_sales = X_test["bitcoins"]
    total_volume = bitcoin_sales.sum()

    true_wash_pct = bitcoin_sales[y_true == 1].sum() / total_volume * 100
    pred_wash_pcts = (bitcoin_sales.values @ Y_pred_matrix) / \
        total_volume * 100
...
tqdm.write(f"\nMt. Gox processing starts:")

# Load data with specified dtypes and columns
df = pd.read_csv(market_path, dtype=dtype_dict,
                 usecols=usecols_list, engine="pyarrow")

# ===============================
# Repeated Buyer–Seller Proxy Feature
# ===============================

# Since buyer_id and seller_id are not available,
# we use symmetry in trade activity as a proxy
df["buyer_seller_repeat_score"] = (
    1 - (df["buyer_24h_trade_count"] - df["seller_24h_trade_count"]).abs() /
    (df["buyer_24h_trade_count"] + df["seller_24h_trade_count"] + 1)
)

# Handle any numerical instability
df["buyer_seller_repeat_score"] = df["buyer_seller_repeat_score"].fillna(0)
...
# Define X and y
# Add new feature
features.append("buyer_seller_repeat_score")
...
# Stratified K-Fold
kfold = StratifiedKFold(n_splits=2, shuffle=True, random_state=42)#changed

for model_name in model_lists:
    tqdm.write(f"\n>>> Model: {model_name}")

    for fold_idx, (train_idx, test_idx) in enumerate(tqdm(kfold.split(X, y), desc=f"Folds: {model_name}"), 1):
        tqdm.write(f"\n--- Fold {fold_idx} ---")
        ...
        result = model_func(X_train, y_train, X_test, y_test, feature_names)
